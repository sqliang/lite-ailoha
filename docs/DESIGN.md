...
CREATE TABLE confirmed_actions (
    ...
    summary    TEXT,                -- 卡片摘要
    fields     TEXT DEFAULT '{}',   -- 结构化字段 JSON
    status     TEXT,                -- confirmed | cancelled | executed
    created_at TEXT DEFAULT (datetime('now'))
);
```
...
本文档描述的 v2 架构已全部实现。关键要点：

- Coordinator 使用 LLM_MODEL（通过 `llm_factory.get_text_llm()`）
- 子 Agent 拆分到 `subagents/` 目录（meeting/contact/reminder）
- Session 状态机已实现（`session_state` 字段贯穿 SSE 事件）
- 两阶段人在回路已实现（阶段一 analyze → 阶段二 insight）
- `actionCard` 包含 `fields` 结构化字段（透传 tool 输出）
- `confirmed_actions` 表含 `fields` 列
- iOS `DeviceDataProvider` 用 `fields` 映射系统 API（CNContact/EKEvent/EKReminder）
